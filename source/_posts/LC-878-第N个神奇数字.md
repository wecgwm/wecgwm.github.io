...
显然通过容斥定理可以得出：
$x$ = $x$ 前面的数为 $a$ 的倍数 + $x$ 前面的数为 $b$ 的倍数 - $x$ 前面的数为 $lcm(a, b)$ 的个数
也就是

$x = \lceil \frac{x}{a} \rceil + \lceil \frac{x}{b} \rceil - \lceil \frac{x}{lcm} \rceil$
...
$x = \lfloor \frac{x}{a} \rfloor + \lfloor \frac{x}{b} \rfloor - \lfloor \frac{x}{lcm} \rfloor$
